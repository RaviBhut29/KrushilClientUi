...
import { getCategoryWiseServiceDetails, getPlan } from "../../FlysesApi/Plan";
...
const Plan = () => {
  ...
  const getCategoryServiceDetails = async () => {
    setLoadingStatus(true);
    const categoryId = await decryptWithRk(splitdata[splitdata.length - 1]);
    getCategoryWiseServiceDetails(categoryId)
      .then(async (response) => {
        if (response) {
          const serviceId = await encrptWithRk(
            response?.ctService
          );
          setServiceDetails({
            srName: response?.srName,
  ...
  const siteMapPath = [
    ...
    {
      name: serviceDetails?.srName,
      clickable: true,
      isHome: false,
      path: `/services/${serviceDetails?.srName.replace(/ /g, "-").toLowerCase()}/${serviceDetails?.ctService}`,
    },
    {
      name: serviceDetails?.ctName,
      clickable: true,
      isHome: false,
      path: `/services/${splitdata[splitdata.length - 4]}/${
